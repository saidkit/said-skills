...
3. Append `## Debrief close` footer to `<feature-id>.tasks.md` (after `## Lessons (<feature-id>)` if present):
   ...
   - Gate-skip: N/A (or: Phase-3.5 gates skipped per operator YYYY-MM-DD — <reason>, per Step-0 item 6)
   ```
