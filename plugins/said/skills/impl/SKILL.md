...
1. **Resolve mode + feature-id + tasks file** via SAID-canonical disambiguation:
   ...
   - Single arg matching `^[A-Z]+-\d+$` (e.g., `<FEAT>-NN`) AND a tasks file resolves (bare `<arg>.tasks.md` OR slug-suffixed `<arg>-*.tasks.md` via the resolver below) → **whole-feature mode**; build batch = every `Status: Todo` heading in file order.
